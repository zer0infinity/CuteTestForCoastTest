...
def generate(env, **kw):
    ...
    StanfordUtils.DependsOn(env, 'zlib')
...
#    env.AppendUnique(CPPDEFINES =['ONLY_STD_IOSTREAM'])
    ...
    if not kw.get('depsOnly', 0):
        ...
        env.Tool('registerObjects', package=_packagename, includes=StanfordUtils.listFiles([os.path.join(_includeSubdir, 'ZipStream.h'), os.path.join(_includeSubdir, 'config_compress.h')]))
