...
namespace NTLMCoder {
	long decodeshort(const String &dm, long pos) {
		return (unsigned char) dm[pos] + ((unsigned char) dm[pos + 1L] << 8);
	}
	void appendshort(String &buff, unsigned short val) {
		buff.Append((char) (val & 0xff)).Append((char) (val >> 8));
	}
	void appendlong(String &buff, long val) {
		appendshort(buff, val & 0xffff);
		appendshort(buff, (val >> 16) & 0xffff);
	}
	String MakeBase64(const String &buff) {
	...
	bool DecodeClientMsg(const String &base64msg, String &domain, String &host) {
		...
		String dm;
		if (DecodeBase64(dm, base64msg) && "NTLMSSP" == dm.SubString(0L, 7L) && '\x01' == dm[8L] && '\x03' == dm[12L]
				&& '\xb2' == dm[13L]) {
	...
	String EncodeServerNonce(String nonce, long flags) {
	 ...
	 */
	void setup_des_key(const String &keystr, DES_key_schedule *ks) {
		DES_cblock key;
	...
	String cutfrommessage(const String dm, long lengthpos) {
	...
	bool DecodeResponse(const String &base64msg, const String &nonce, const String &lmhash, const String &nthash, String &domain,
		...
		if (DecodeBase64(dm, base64msg) && "NTLMSSP" == dm.SubString(0L, 7L) && '\x03' == dm[8L] && '\x01' == dm[60L] && '\x82' == dm[61L]
			...
			return CalculateNonceHash(lmhash, nonce) == cutfrommessage(dm, 12L)
					&& CalculateNonceHash(nthash, nonce) == cutfrommessage(dm, 20L);
		}
		return false;
	}
	void appendlenoff(String &buff, long length, short off) {
