...
class EXPORTDECL_WDBASE Session : public NotCloned, public virtual LookupInterface
{
public:
	//!sessions are generated by factory methods of a server object
	//! \param server backlink to server object that uses this session
	//! \param name name of the session not very important since a session has no configuration yet
	Session(const char *name, Context &ctx);
	...
	//! \param env legacy name, content similar to a CGI's environment
	//! with things like REMOTE_ADDR etc.
	//! \param query the query content of the request, what was passed as
	//! explicit request parameters and put there by the Requestprocessor
...
class EXPORTDECL_WDBASE SessionInfo : public Action
{
public:
	//--- constructors
