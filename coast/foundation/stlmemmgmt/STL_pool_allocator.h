...
namespace STLStorage
{
	...
	public:
		...
		*/
		pool_allocator()
			: fpIntPool() {
			_StartTrace1(pool_allocator.pool_allocator, "this:" << (long)this << " sizeof(T):" << sizeof(T));
		...
		pool_allocator(const ThisType &aAllocator)
			...
			_StartTrace1(pool_allocator.pool_allocator, "copy this:" << (long)this << " sizeof(T):" << sizeof(T) << " other:" << (long)&aAllocator << " sizeof(other):" << sizeof(typename ThisType::value_type));
			_Trace("IntPool @" << (long)fpIntPool.get());
		}
		...
		pool_allocator(const pool_allocator<U, UserAllocator, NextSize>& aAllocator)
			: fpIntPool() {
			_StartTrace1(pool_allocator.pool_allocator, "copy other type this:" << (long)this << " sizeof(T):" << sizeof(T) << " other:" << (long)&aAllocator << " sizeof(U):" << sizeof(typename pool_allocator<U, UserAllocator, NextSize>::value_type));
		...
		pointer allocate(const size_type n) throw(std::bad_alloc) {
			...
			_StatTrace(pool_allocator.allocate_n, "this:" << (long)this << " ptr:" << (long)ptr << " size:" << (long)n << '*' << sizeof(T) << " calling ordered_malloc " << " fpIntPool:" << (long)fpIntPool.operator->(), Coast::Storage::Current());
			if (ptr == 0) {
				throw std::bad_alloc();
			}
			return ptr;
		...
		pointer allocate(const size_type n, const void *const) throw(std::bad_alloc) {
			_StatTrace(pool_allocator.allocate_n_void, "this:" << (long)this << " size:" << (long)n << '*' << sizeof(T) << " void*", Coast::Storage::Current());
			return allocate(n);
		}
		void deallocate(const pointer ptr, const size_type n) {
			_StatTrace(pool_allocator.deallocate_ptr_n, "this:" << (long)this << " ptr:" << (long)ptr << " size:" << (long)n << '*' << sizeof(T) << " calling ordered_free " << " fpIntPool:" << (long)fpIntPool.operator->(), Coast::Storage::Current());
