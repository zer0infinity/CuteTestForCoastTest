...
public:
	...
	//! \param mode ios modes, bitwise or of in, out, app, ate
	...
	//! \param mode ios modes, bitwise or of in, out, app, ate
	//! if mode is ios::app or ios::ate output is appended
	...
	//! <B>do not change it when continuing using the stream</B>
	String &str();
...
private:
	...
	//! enlarge the underlying String, adjust buffer pointers if needed
	bool reserve ( long newlength ); // expand file size and mapped region
	//! initialize get and put pointers of streambuf
	void initGetPut();       // set internal put und get buffer and position
...
{
public:
	//! ctor, take String pointed to by s to read the input
	//! direct input taken from string
