...
typedef std::iterator<std::random_access_iterator_tag, char const> ConstStringIteratorBase;
...
protected:
	const String * a;
	long position;
	String_const_iterator(const String * s, long p = 0) :
		a(s), position(p) {
	}
public:
	...
	String_const_iterator &operator+=(difference_type index) {
		position += index;
		return *this;
	}
	String_const_iterator &operator-=(difference_type index) {
