...
#ifndef _AllocatorPool_H
#define _AllocatorPool_H
...
template <typename UserAllocator, typename SizeT>
class pool: protected boost::simple_segregated_storage<SizeT>
{
  public:
  ...
  protected:
    boost::details::PODptr<size_type> list;

    boost::simple_segregated_storage<size_type> & store() { return *this; }
    const boost::simple_segregated_storage<size_type> & store() const { return *this; }
    const size_type requested_size;
    size_type next_size;
    size_type start_size;

    // finds which POD in the list 'chunk' was allocated from
    boost::details::PODptr<size_type> find_POD(void * const chunk) const;
...
template <typename UserAllocator, typename SizeT>
bool pool<UserAllocator, SizeT>::release_memory()
{
  ...
  boost::details::PODptr<size_type> ptr = list;
  boost::details::PODptr<size_type> prev;
  ...
  while (ptr.valid())
  {
    ...
    const boost::details::PODptr<size_type> next = ptr.next();
...
//      UserAllocator::free(ptr.begin());
      static_cast<UserAllocator*>(this)->free(ptr.begin());
      ret = true;
    }
...
template <typename UserAllocator, typename SizeT>
bool pool<UserAllocator, SizeT>::purge_memory()
{
  ...
  {
    // hold "next" pointer
    const boost::details::PODptr<size_type> next = iter.next();

    // delete the storage
    //TODO replace with CRTP
    static_cast<UserAllocator*>(this)->free(iter.begin());
...
template <typename UserAllocator, typename SizeT>
void * pool<UserAllocator, SizeT>::malloc_need_resize()
{
...
//  char * const ptr = UserAllocator::malloc(POD_size);
  char * const ptr = static_cast<UserAllocator*>(this)->malloc(POD_size);
  if (ptr == 0)
    return 0;
  const boost::details::PODptr<size_type> node(ptr, POD_size);
  next_size <<= 1;
...
template <typename UserAllocator, typename SizeT>
void * pool<UserAllocator, SizeT>::ordered_malloc_need_resize()
{
  // No memory in any of our storages; make a new storage,
  const size_type partition_size = alloc_size();
  const size_type POD_size = next_size * partition_size +
...
//  char * const ptr = UserAllocator::malloc(POD_size);
  ...
  {
	boost::details::PODptr<size_type> prev = list;
...
template <typename UserAllocator, typename SizeT>
void * pool<UserAllocator, SizeT>::ordered_malloc(const size_type n)
{
  const size_type partition_size = alloc_size();
  const size_type total_req_size = n * requested_size;
  const size_type num_chunks = total_req_size / partition_size +
...
template <typename UserAllocator, typename SizeT>
boost::details::PODptr<typename pool<UserAllocator, SizeT>::size_type>
pool<UserAllocator, SizeT>::find_POD(void * const chunk) const
{
...
class MyPool : public pool<MyPool, std::size_t> {
	Allocator *fAllocator;
public:

	MyPool(Allocator* allocator, size_type nrequested_size, size_type nnext_size = 32) :
		pool<MyPool, std::size_t>(nrequested_size, nnext_size), fAllocator(allocator) {}

	char * malloc(size_t size) {
		return reinterpret_cast<char*>(fAllocator->Malloc(size));
	}

	void free(void * const chunk) {
		fAllocator->Free(chunk);
	}
    void * pmalloc()
    {
      return pool<MyPool, std::size_t>::malloc();
    }

    // pre: 'chunk' must have been previously
    //        returned by *this.malloc().
    void pfree(void * const chunk) {
      return pool<MyPool, std::size_t>::free(chunk);
    }
};
...
#endif		//not defined _AllocatorPool_H
