...
AnythingParserTest::AnythingParserTest() {
	Anything undefAny;
	emptyAny = undefAny;
	anyTemp0 = undefAny;
	anyTemp1 = undefAny;
...
void AnythingParserTest::runAllTests(cute::suite &s) {
	s.push_back(CUTE_SMEMFUN(AnythingParserTest, parseSimpleTypeCharPtr));
	s.push_back(CUTE_SMEMFUN(AnythingParserTest, parseSimpleTypeDouble));
	s.push_back(CUTE_SMEMFUN(AnythingParserTest, parseSimpleTypeLong));
	s.push_back(CUTE_SMEMFUN(AnythingParserTest, parseSimpleTypeNull));
	s.push_back(CUTE_SMEMFUN(AnythingParserTest, parseNumber));
	s.push_back(CUTE_SMEMFUN(AnythingParserTest, parseOctal));
	s.push_back(CUTE_SMEMFUN(AnythingParserTest, parseBinary));
	s.push_back(CUTE_SMEMFUN(AnythingParserTest, parseMixedAnything));
	s.push_back(CUTE_SMEMFUN(AnythingParserTest, importEmptyStream));
	s.push_back(CUTE_SMEMFUN(AnythingParserTest, readWriteAnything));
	s.push_back(CUTE_SMEMFUN(AnythingParserTest, parseTestFiles));
	s.push_back(CUTE_SMEMFUN(AnythingParserTest, testObjectParsing));
	s.push_back(CUTE_SMEMFUN(AnythingParserTest, testNumberParsing));
}
...
void AnythingParserTest::IntParseSimpleTypeLong(const String &inp, long exp) { //lint !e578
	...
	anyTest.Import(is0);
	ASSERT_EQUAL(AnyLongType, anyTest.GetType());
	ASSERT_EQUAL(exp, anyTest.AsLong());
...
void AnythingParserTest::parseSimpleTypeLong() {
	...
	anyTest.Import(isx);
	ASSERT_EQUAL(0xfffffffe, static_cast<unsigned long>(anyTest.AsLong(0)));	//lint !e569
	ASSERT_EQUAL(AnyLongType, anyTest.GetType());
	ASSERT_EQUAL(1, anyTest.GetSize());
}
...
void AnythingParserTest::parseSimpleTypeDouble() {
	...
	ASSERT_EQUAL(AnyDoubleType, anyTest.GetType());
	ASSERT_EQUAL(1, anyTest.GetSize());

	String input1 = "1.0123456789E+300";
	IStringStream is1(input1);
	anyTest.Import(is1);
	ASSERT_EQUAL_DELTA(1.0123456789E+300, anyTest.AsDouble(), 0.00001);
	ASSERT_EQUAL(AnyDoubleType, anyTest.GetType());
	ASSERT_EQUAL(1, anyTest.GetSize());
	...
	IStringStream is5(input5);
	anyTest.Import(is5);
	ASSERT_EQUAL_DELTA(doubleDefault, anyTest.AsDouble(doubleDefault), 0.00001);
	ASSERT_EQUAL(AnyNullType, anyTest.GetType());
	...
	anyTest.Import(is9);
	ASSERT_EQUAL_DELTA(1.0123456789E+20, anyTest.AsDouble(), 0.00001);
	ASSERT_EQUAL(AnyDoubleType, anyTest.GetType());
	ASSERT_EQUAL(1, anyTest.GetSize());
}
...
void AnythingParserTest::importEmptyStream() {
	...
	Anything any0;
	any0.Import(is);
	ASSERT_EQUAL(0, any0.GetSize());
	ASSERT_EQUAL(AnyNullType, any0.GetType());
}
 ...
 */
{
	...
	is >> any;
	ASSERT_EQUAL(AnyDoubleType, any.GetType());
	if (AnyDoubleType== any.GetType()) {
		ASSERT_EQUAL_DELTA(any.AsDouble(), val, 1E-8);
	} else {
		ASSERT_EQUAL(-1, id);
 ...
 */
{
	...
	is >> any;
	ASSERT_EQUAL(AnyLongType, any.GetType());
	if (AnyLongType== any.GetType()) {
		ASSERT_EQUAL(val, any.AsLong());
	} else {
		ASSERT_EQUAL(-1, id);
		// crutch until we can add some info, identify whats wrong
	} // if
 ...
 */
{
	...
	is >> any;
	ASSERT_EQUAL(AnyCharPtrType, any.GetType());
	if (AnyCharPtrType== any.GetType()) {
		ASSERT_EQUAL(in, any.AsCharPtr());
	} else {
		ASSERT_EQUAL(-1, id);
		// crutch until we can add some info, identify whats wrong
	} // if
...
void AnythingParserTest::parseSimpleTypeCharPtr() {
	...
	IStringStream is0(input0);
	anyTest.Import(is0);
	ASSERT_EQUAL(AnyCharPtrType, anyTest.GetType());		// Because of the L at the end
	ASSERT_EQUAL(1, anyTest.GetSize());
	if (anyTest.AsCharPtr()) {
		ASSERT_EQUAL(anyTest.AsCharPtr(), "1234567890L");
	}
	...
	IStringStream is1(input1);
	anyTest.Import(is1);
	ASSERT_EQUAL(AnyCharPtrType, anyTest.GetType());
	ASSERT_EQUAL(1, anyTest.GetSize());
	if (anyTest.AsCharPtr()) {
		ASSERT_EQUAL(anyTest.AsCharPtr(), "aAbB01234");
	}
	...
	ASSERT_EQUAL(1, anyTest.GetSize());
	if (anyTest.AsCharPtr()) {
		ASSERT_EQUAL(anyTest.AsCharPtr(), "aAbB01");
	}
	...
	ASSERT_EQUAL(1, anyTest.GetSize());
	for (i = 1; i < 256; i++) {
		ASSERT_EQUAL((char )i, anyTest.AsCharPtr()[i - 1]);
	}
	...
	for (i = 1; i < 256; i++) {
		ASSERT_EQUAL((unsigned char )i, (unsigned char )(anyTest.AsCharPtr()[i - 1]));
	}
	...
	ASSERT_EQUAL(0, anyTest.GetSize());
	if (anyTest.AsCharPtr()) {
		ASSERT_EQUAL(anyTest.AsCharPtr(), "!abc");
	}
	ASSERT_EQUAL(AnyNullType, anyTest.GetType());
	ASSERT_EQUAL(0, anyTest.GetSize());
}
...
void AnythingParserTest::parseMixedAnything() {
	...
	if (osHlp) {
		*osHlp << anyAnew;
		...
		ASSERT_EQUAL("'write tmp/AnyTest.txt'", "'could not write tmp/AnyTest.txt'");
	}

	std::istream *isHlp = system::OpenIStream("tmp/AnyTest", "txt");
	if (!isHlp) {
		ASSERT_EQUAL("'read tmp/AnyTest.txt'", "'could not read tmp/AnyTest.txt'");
	} else {	// if ( isHlp )
		Anything anyHlp;
		anyHlp.Import(*isHlp);
		delete isHlp;
		...
		if (osNew) {
			*osNew << anyHlp;
			...
			ASSERT_EQUAL("'write tmp/AnyTestNew.txt'", "'could not write tmp/AnyTestNew.txt'");
		}

		anyHlp["A"].SlotName(0L);
		ASSERT_EQUAL(AnyLongType, anyHlp["A"].At(0L).GetType());
		ASSERT_EQUAL(1, anyHlp["A"].At(0L).GetSize());
		ASSERT_EQUAL(123, anyHlp["A"].At(0L).AsLong());

		anyHlp["A"].SlotName(1L);
		ASSERT_EQUAL(AnyCharPtrType, anyHlp["A"].At(1L).GetType());
		ASSERT_EQUAL(1, anyHlp["A"].At(1L).GetSize());
		if (anyHlp["A"].At(1L).AsCharPtr()) {
			ASSERT_EQUAL(anyHlp["A"].At(1L).AsCharPtr(), "123L");
		}

		anyHlp["A"].SlotName(2L);
		ASSERT_EQUAL(AnyCharPtrType, anyHlp["A"].At(2L).GetType());
		ASSERT_EQUAL(1, anyHlp["A"].At(2L).GetSize());
		if (anyHlp["A"].At(2L).AsCharPtr()) {
			ASSERT_EQUAL(anyHlp["A"].At(2L).AsCharPtr(), "String");
		}

		//anyHlp["A"].At[3].SlotName(0L);
		ASSERT_EQUAL(AnyLongType, anyHlp["A"].At(3L).At(0L).GetType());
		ASSERT_EQUAL(1, anyHlp["A"].At(3L).At(0L).GetSize());
		ASSERT_EQUAL(0, anyHlp["A"].At(3L).At(0L).AsLong());

		//anyHlp["A"].At[3].SlotName(1L);
		ASSERT_EQUAL(AnyLongType, anyHlp["A"].At(3L).At(1L).GetType());
		ASSERT_EQUAL(1, anyHlp["A"].At(3L).At(1L).GetSize());
		ASSERT_EQUAL(1, anyHlp["A"].At(3L).At(1L).AsLong());

		//anyHlp["A"].At[3].SlotName(2L);
		ASSERT_EQUAL(AnyLongType, anyHlp["A"].At(3L).At(2L).GetType());
		ASSERT_EQUAL(1, anyHlp["A"].At(3L).At(2L).GetSize());
		ASSERT_EQUAL(2, anyHlp["A"].At(3L).At(2L).AsLong());

		//anyHlp["A"].At[3].SlotName(3L);
		ASSERT_EQUAL(AnyLongType, anyHlp["A"].At(3L).At(3L).GetType());
		ASSERT_EQUAL(1, anyHlp["A"].At(3L).At(3L).GetSize());
		ASSERT_EQUAL(3, anyHlp["A"].At(3L).At(3L).AsLong());

		//anyHlp["A"].At[3].SlotName(4L);
		ASSERT_EQUAL(AnyLongType, anyHlp["A"].At(3L).At(4L).GetType());
		ASSERT_EQUAL(1, anyHlp["A"].At(3L).At(4L).GetSize());
		ASSERT_EQUAL(4, anyHlp["A"].At(3L).At(4L).AsLong());

		anyHlp["A"].At(3L).At(5L)["B"].SlotName(0L);
		ASSERT_EQUAL(AnyCharPtrType, anyHlp["A"].At(3L).At(5L)["B"].At(0L).GetType());
		ASSERT_EQUAL(1, anyHlp["A"].At(3L).At(5L)["B"].At(0L).GetSize());
		if (anyHlp["A"].At(3L).At(5L)["B"].At(0L).AsCharPtr()) {
			ASSERT_EQUAL(anyHlp["A"].At(3L).At(5L)["B"].At(0L).AsCharPtr(), "ABC");
		}

		anyHlp["A"].At(3L).At(5L)["B"].SlotName(1L);
		ASSERT_EQUAL(AnyCharPtrType, anyHlp["A"].At(3L).At(5L)["B"].At(1L).GetType());
		ASSERT_EQUAL(1, anyHlp["A"].At(3L).At(5L)["B"].At(1L).GetSize());
		if (anyHlp["A"].At(3L).At(5L)["B"].At(1L).AsCharPtr()) {
			ASSERT_EQUAL(anyHlp["A"].At(3L).At(5L)["B"].At(1L).AsCharPtr(), "BCD");
		}

		anyHlp["A"].At(3L).At(5L)["B"].SlotName(2L);
		ASSERT_EQUAL(AnyCharPtrType, anyHlp["A"].At(3L).At(5L)["B"].At(2L).GetType());
		ASSERT_EQUAL(1, anyHlp["A"].At(3L).At(5L)["B"].At(2L).GetSize());
		if (anyHlp["A"].At(3L).At(5L)["B"].At(2L).AsCharPtr()) {
			ASSERT_EQUAL(anyHlp["A"].At(3L).At(5L)["B"].At(2L).AsCharPtr(), "CDE");
		}

		anyHlp["A"].At(3L).At(5L)["B"].At(3L)["C"]["D"]["E"].SlotName(0L);
		ASSERT_EQUAL(AnyLongType, anyHlp["A"].At(3L).At(5L)["B"].At(3L)["C"]["D"]["E"].At(0L).GetType());
		ASSERT_EQUAL(1, anyHlp["A"].At(3L).At(5L)["B"].At(3L)["C"]["D"]["E"].At(0L).GetSize());
		ASSERT_EQUAL(1, anyHlp["A"].At(3L).At(5L)["B"].At(3L)["C"]["D"]["E"].At(0L).AsLong());

		anyHlp["A"].At(3L).At(5L)["B"].At(3L)["C"]["D"]["E"].SlotName(1L);
		ASSERT_EQUAL(AnyLongType, anyHlp["A"].At(3L).At(5L)["B"].At(3L)["C"]["D"]["E"].At(1L).GetType());
		ASSERT_EQUAL(1, anyHlp["A"].At(3L).At(5L)["B"].At(3L)["C"]["D"]["E"].At(1L).GetSize());
		ASSERT_EQUAL(2, anyHlp["A"].At(3L).At(5L)["B"].At(3L)["C"]["D"]["E"].At(1L).AsLong());

		ASSERT_EQUAL(AnyDoubleType, anyHlp["A"].At(3L).At(5L)["B"].At(3L)["C"]["F"].GetType());
		ASSERT_EQUAL(1, anyHlp["A"].At(3L).At(5L)["B"].At(3L)["C"]["F"].GetSize());
		ASSERT_EQUAL_DELTA(1e34, anyHlp["A"].At(3L).At(5L)["B"].At(3L)["C"]["F"].AsDouble(), 0.00001);

		ASSERT_EQUAL(AnyLongType, anyHlp["G"].GetType());
		ASSERT_EQUAL(1, anyHlp["G"].GetSize());
		ASSERT_EQUAL(123, anyHlp["G"].AsLong());

		ASSERT_EQUAL(AnyLongType, anyHlp["H"].GetType());
		ASSERT_EQUAL(1, anyHlp["H"].GetSize());
		ASSERT_EQUAL(1234, anyHlp["H"].AsLong());
		if (anyHlp["I"].AsCharPtr()) {
			ASSERT_EQUAL(anyHlp["I"].AsCharPtr(), "String");
		}
	}
}
...
void AnythingParserTest::testObjectParsing() {
	{
		...
		ASSERT_EQUAL("myObjectImpl", any.SlotName(0));
		ASSERT_EQUAL(12345L, reinterpret_cast<long>(any["myObjectImpl"].AsIFAObject(0)));
	}
	{
		String buf("{/myObjectImpl &-12345}");
		IStringStream is(buf);
	...
	}
	{
		...
		ASSERT(any.Import(is));
		ASSERT_EQUAL("myObjectImpl", any.SlotName(0));
		ASSERT_EQUAL(4294967295UL, reinterpret_cast<unsigned long>(any["myObjectImpl"].AsIFAObject(0)));
	}
	myObject aObj;
	{
		...
		ASSERT(any.Import(is));
		ASSERT_EQUAL("myObjectImpl", any.SlotName(0));
		ASSERT_EQUAL((IFAObject * )&aObj, any["myObjectImpl"].AsIFAObject(0));
	}
	{
		String buf("{/myObjectImpl &");
		...
		ASSERT_EQUAL("myObjectImpl", any.SlotName(0));
		ASSERT_EQUAL((IFAObject * )&aObj, any["myObjectImpl"].AsIFAObject(0));
		ASSERT_EQUAL("some", any.SlotName(1));
		ASSERT_EQUAL("content", any["some"].AsCharPtr());
	}
}
...
void AnythingParserTest::testNumberParsing() {
	{
		...
		Anything any;
		ASSERT(any.Import(is));
		ASSERT_EQUAL("myLong", any.SlotName(0));
		ASSERT_EQUAL(12345L, any["myLong"].AsLong(0));
	}
	{
