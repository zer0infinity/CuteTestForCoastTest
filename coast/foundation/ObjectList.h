...
public:
	...
	virtual bool InsertTail(const typename std::list<Tp>::value_type &newObjPtr) {
		StartTrace(ObjectList.InsertTail);
		if ( !IsShuttingDown() ) {
			push_back(newObjPtr);
	...
	virtual bool RemoveHead(typename std::list<Tp>::reference aElement, long lSecs = 0L, long lNanosecs = 0L) {
		StartTrace(ObjectList.RemoveHead);
	...
	inline virtual void SignalShutdown(bool bDestructive = false) {
		StartTrace1(ObjectList.SignalShutdown, (bDestructive ? "destructive" : ""));
	...
	inline virtual void ResetAfterShutdown() {
		fShutdown = false;
		fDestructiveShutdown = false;
	}

protected:
	virtual void IntRemoveHead(typename std::list<Tp>::reference aElement) {
		aElement = this->front();
		this->pop_front();
	}
	...
	inline virtual bool IntIsEmpty() const {
		return this->empty();
	}
